...
jQuery( document ).on( 'action.ready_critique', function() {
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.critique_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		critique_set_cookie( 'critique_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// On first run
	critique_woocommerce_add_inc_dec();
	...
	// Add buttons inc/dec
	function critique_woocommerce_add_inc_dec() {
		jQuery( '.woocommerce div.quantity:not(.inited),.woocommerce-page div.quantity:not(.inited)' ).each( function() {
			...
			$self
				...
				.on( 'click', '>span', function(e) {
					critique_woocomerce_inc_dec_click( jQuery( this ) );
					e.preventDefault();
					return false;
				} );
		} );
	...
	function critique_woocomerce_inc_dec_click(button) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		if ( table_cart.length > 0 ) {
			critique_woocommerce_update_cart( 'update' );
		}
	} );
	...
	if ( table_cart.length > 0 ) {
		critique_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			critique_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function critique_woocommerce_update_cart(status){
		setTimeout(
			function() {
				...
				if ( w >= CRITIQUE_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
