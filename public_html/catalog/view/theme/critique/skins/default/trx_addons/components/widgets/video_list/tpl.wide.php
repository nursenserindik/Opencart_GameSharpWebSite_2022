<?php
/**
 * The style "wide" of the Widget "Video list"
 *
...
if ( is_array( $videos ) && count( $videos ) > 0 && ( ! empty( $videos[0]['video_url'] ) || ! empty( $videos[0]['video_embed'] ) ) ) {
	...
	// Widget body
	?><div class="trx_addons_video_list trx_addons_video_list_<?php echo esc_attr( $args['controller_style'] ); ?> trx_addons_video_list_controller_right trx_addons_video_list_type_<?php echo esc_attr( $args['type'] ); ?>"><?php
		// Video frame
		?><div class="trx_addons_video_list_video_wrap"><?php
			...
			trx_addons_show_layout( trx_addons_get_video_layout( array(
																...
																'cover_size' => apply_filters( 'trx_addons_filter_video_list_thumb_size', 'w-video-wide-main' ),
																)
															)
								);
				...
				if ( empty( $controller_content ) ) {
					if ( ! empty( $v['image'] ) ) {
						$controller_content .= '<div class="trx_addons_video_list_image">'
													. '<img src="' . esc_url( trx_addons_get_attachment_url( $v['image'], critique_get_thumb_size( 'w-video-wide-small' ) ) ) . '" alt="' . esc_attr( $v['title'] ) . '">'
												. '</div>';
					}
