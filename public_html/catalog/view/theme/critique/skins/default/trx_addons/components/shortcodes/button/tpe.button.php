...
#><div id="{{ id }}" class="sc_item_button sc_button_wrap sc_align_{{ settings.align }}"><#
	...
	_.each(settings.buttons, function(item) {
		...
		if ( item.type == 'rounded' ) item.hover_style = item.hover_style_rounded;
		...
		#><a href="{{ item.link.url }}"<#
			...
			#> class="<# print(button_classes
								...
								+ (typeof item.hover_style != 'undefined' && item.hover_style != '' && item.hover_style != 'default' ? ' hover_style_' + item.hover_style : '')								
	                       	    + (item.subtitle != '' ? ' with_subtitle' : '')
	                            ); #>"<#
		...
		#>><#
			...
			if (!trx_addons_is_off(item.image.url) || !trx_addons_is_off(icon)) {
				#><span class="sc_button_icon"><#
					if (item.image.url != '') {
						#><img class="sc_icon_as_image" src="{{ item.image.url }}" alt="<?php esc_attr_e('Icon', 'critique'); ?>"><#
					} else if (icon.indexOf('//') >= 0) {
						if (icon.indexOf('.svg') >= 0) {
							#><object type="image/svg+xml" data="{{ icon }}" border="0"></object><#
						} else {
							#><img class="sc_icon_as_image" src="{{ icon }}" alt="<?php esc_attr_e('Icon', 'critique'); ?>"><#
						}
					} else {
