<?php
/**
 * The style "news" of the Widget "Video list"
 *
...
if ( is_array( $videos ) && count( $videos ) > 0 && ( ! empty( $videos[0]['video_url'] ) || ! empty( $videos[0]['video_embed'] ) ) ) {
	...
	// Widget body
	?><div class="trx_addons_video_list trx_addons_video_list_<?php echo esc_attr( $args['controller_style'] ); ?> trx_addons_video_list_controller_bottom trx_addons_video_list_type_<?php echo esc_attr( $args['type'] ); ?>"><?php
		// Video frame
		?><div class="trx_addons_video_list_video_wrap"><?php
			...
			trx_addons_show_layout( trx_addons_get_video_layout( array(
																...
																'cover_size' => apply_filters( 'trx_addons_filter_video_list_thumb_size', 'w-video-news-main' ),
																)
															)
								);
				...
				if ( empty( $controller_content ) ) {
					if ( ! empty( $v['image'] ) ) {
						$controller_content .= '<div class="trx_addons_video_list_image">'
													. '<img src="' . esc_url( trx_addons_get_attachment_url( $v['image'], trx_addons_get_thumb_size( 'w-video-news-small' ) ) ) . '" alt="' . esc_attr( $v['title'] ) . '">'
												. '</div>';
					}
					if ( ! empty( $v['title'] ) ) {
