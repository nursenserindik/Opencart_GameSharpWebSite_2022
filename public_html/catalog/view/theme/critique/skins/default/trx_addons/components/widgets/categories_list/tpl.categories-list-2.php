			...
			if ($show_thumbs) {
				...
				?><div class="categories_list_image"><?php
					$src = empty($image) 
								...
								: trx_addons_add_thumb_size($image, critique_get_thumb_size('med-5'));
					$attr = trx_addons_getimagesize($src);
					?><img src="<?php echo esc_url($src); ?>" <?php if (!empty($attr[3])) trx_addons_show_layout($attr[3]); ?> alt="<?php esc_attr_e('Category image', 'critique'); ?>">
				</div><?php
			}
			?><h6 class="categories_list_title">
				<span class="categories_list_item_number"><?php echo esc_html($i); ?></span>
