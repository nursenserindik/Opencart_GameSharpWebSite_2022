				...
				?><div class="categories_list_image"><?php
					if ( !empty($icon) && !trx_addons_is_off($icon) ) {
						...
						$src = empty($image)
									...
									: trx_addons_add_thumb_size( $image, trx_addons_get_thumb_size('tiny') );
						$attr = trx_addons_getimagesize($src);
						?><img src="<?php echo esc_url($src); ?>" <?php if (!empty($attr[3])) trx_addons_show_layout($attr[3]); ?> alt="<?php esc_attr_e('Category image', 'critique'); ?>"><?php
					}
				?></div><?php
			}
			?>
			<div class="categories_list_title_wrapper">
				<span class="categories_list_title"><?php echo esc_html($cat->name); ?></span>
				<?php
