...
if (trx_addons_is_on($args['mobile_button']) && !$is_complex_menu) {
	?><div class="sc_layouts_iconed_text sc_layouts_menu_mobile_button">
		<a class="sc_layouts_item_link sc_layouts_iconed_text_link" href="#">
			...
			<span class="sc_layouts_iconed_text_text"><?php echo esc_html__('Menu', 'critique'); ?></span>
		</a>
	</div><?php
	trx_addons_sc_layouts_showed('menu_button', true);
}

$trx_addons_menu = !empty($args['location']) || !empty($args['menu']) ? critique_skin_get_nav_menu($args['location'], $args['menu']) : '';
if (!empty($trx_addons_menu)) {
	...
	if ($is_complex_menu) {
		...
		?><nav class="<?php echo esc_attr($args['direction'] == 'horizontal' ? 'sc_layouts_menu ' : ''); ?>sc_layouts_menu_<?php
