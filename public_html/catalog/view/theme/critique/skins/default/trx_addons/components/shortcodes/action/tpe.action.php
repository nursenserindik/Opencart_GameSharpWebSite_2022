...
var link_class = "<?php echo apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_action_item_link sc_button  sc_button_default sc_button_size_normal hover_style_1 color_style_2 sc_button_with_icon sc_button_icon_left', 'sc_action'); ?>";
...
#><div id="{{ id }}" class="<# print( trx_addons_apply_filters('trx_addons_filter_sc_classes', 'sc_action sc_action_'+settings.type, settings ) ); #>">
	...
	_.each(settings.actions, function(item) {
		...
		#>><#
			...
			if (item.image.url != '') {
				#><div class="sc_action_item_image"><img src="{{ item.image.url }}" alt="<?php esc_attr_e("Action's image", 'critique'); ?>"></div><#
			} else {
				...
				if ( icon != '' && icon != 'none' ) {
					...
					#><div class="sc_action_item_icon sc_icon_type_{{ item.icon_type }} {{ icon }}"
						...
						if (svg != '') {
							...
							#><img class="sc_icon_as_image" src="{{ img }}" alt="<?php esc_attr_e('Icon', 'critique'); ?>"><#
						} else {
			...
			if (item.link.url != '' && item.link_text != '') {
				#><a href="{{ item.link.url }}" class="{{ link_class }}">
					<span class="sc_button_icon">
						<span class="icon-plain"></span>
					</span>
					<span class="sc_button_text">
						<span class="sc_button_title">{{ item.link_text }}</span>
					</span>
				</a><#
			}
			if (item.info != '') {
