 ...
 *
 * @package CRITIQUE
 * @since CRITIQUE 1.0.22
 */

// If this theme is a free version of premium theme
if ( ! defined( 'CRITIQUE_THEME_FREE' ) ) {
	define( 'CRITIQUE_THEME_FREE', false );
}
if ( ! defined( 'CRITIQUE_THEME_FREE_WP' ) ) {
	define( 'CRITIQUE_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'CRITIQUE_THEME_ALLOW_UPDATE' ) ) {
	define( 'CRITIQUE_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'CRITIQUE_ALLOW_SKINS' ) ) {
	define( 'CRITIQUE_ALLOW_SKINS', true );
}
if ( ! defined( 'CRITIQUE_DEFAULT_SKIN' ) ) {
	define( 'CRITIQUE_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'CRITIQUE_REMEMBER_SKIN' ) ) {
	define( 'CRITIQUE_REMEMBER_SKIN', false );
}
...
//-------------------------------------------------------------------------
$GLOBALS['CRITIQUE_STORAGE'] = array(
...
if ( ! function_exists( 'critique_get_theme_pro_key' ) ) {
	function critique_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = critique_storage_get( 'theme_pro_key' );
		}
...
// Return engine info
if ( ! function_exists( 'critique_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'critique_trx_updater_get_theme_info' );
	function critique_trx_updater_get_theme_info( $info ) {
		$engine = critique_storage_get( 'engine' );
...
if ( ! function_exists( 'critique_original_theme_slug' ) ) {
	add_filter( 'critique_filter_original_theme_slug', 'critique_original_theme_slug' );
	function critique_original_theme_slug( $theme_slug = '' ) {
		return 'critique';
	}
}
...
if ( ! function_exists( 'critique_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'critique_importer_set_options', 9 );
	function critique_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( critique_get_protocol() . ':' . critique_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( critique_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'critique_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'critique_ocdi_set_options', 9 );
	function critique_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( critique_get_protocol() . ':' . critique_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'critique_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'critique_customizer_theme_setup1', 1 );
	function critique_customizer_theme_setup1() {
		...
		critique_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' for the body style in the Theme Options
																		...
																		// (can be removed if filter 'critique_filter_allow_fullscreen' return false)
