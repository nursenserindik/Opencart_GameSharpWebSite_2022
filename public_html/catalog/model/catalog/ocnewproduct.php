<?php
class ModelCatalogOcnewproduct extends Model
{
    public function getNewProductsInPage($data = array()) {
    ...
    public function getTotalNewProductsPage($limit) {
        $sql = "SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.date_added DESC LIMIT " . $limit;
