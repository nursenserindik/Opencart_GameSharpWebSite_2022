...
class ControllerExtensionModuleOcTabProducts extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->post['module_image']) && is_file(DIR_IMAGE . $this->request->post['module_image'])) {
            $data['thumb'] = $this->model_tool_image->resize($this->request->post['module_image'], 100, 100);
        } elseif (!empty($module_info) && is_file(DIR_IMAGE . $module_info['module_image'])) {
            $data['thumb'] = $this->model_tool_image->resize($module_info['module_image'], 100, 100);
        } else {
