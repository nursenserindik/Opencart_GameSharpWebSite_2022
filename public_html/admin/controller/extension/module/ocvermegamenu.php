...
class ControllerExtensionModuleOCvermegamenu extends Controller {
	...
	public function index() {
		...
		} elseif (!empty($module_info) && isset($module_info['h_show_homepage '])) {
			$data['h_show_home'] = $module_info['h_show_homepage'];
		} else {
			$data['h_show_home'] = 1;
		}
		if (isset($this->request->post['h_menu_depth'])) {
			$data['h_menu_depth'] = $this->request->post['h_menu_depth'];
		} elseif (!empty($module_info) && isset($module_info['h_menu_depth'])) {
			$data['h_menu_depth'] = $module_info['h_menu_depth'];
		} else {
			$data['h_menu_depth'] = 1;
		}
        ...
        if (isset($this->request->post['h_item_number'])) {
	   	...
	   	if (isset($this->request->post['h_menu_level'])) {
			$data['h_menu_level'] = $this->request->post['h_menu_level'];
		} elseif (!empty($module_info) && isset($module_info['h_menu_level'])) {
			$data['h_menu_level'] = $module_info['h_menu_level'];
		} else {
			$data['h_menu_level'] = 1;
		}
